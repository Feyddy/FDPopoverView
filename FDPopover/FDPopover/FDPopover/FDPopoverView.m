...
@interface FDPopoverView () <UITableViewDelegate, UITableViewDataSource>
...
#pragma mark - Data
@property (nonatomic, copy) NSArray<FDPopoverAction *> *actions;
...
- (void)setStyle:(PopoverViewStyle)style {
    _style = style;
    _tableView.separatorColor = [FDPopoverViewCell bottomLineColorForStyle:_style];
...
- (CGFloat)calculateMaxWidth {
    ...
    UIFont *titleFont = [FDPopoverViewCell titleFont];
    for (FDPopoverAction *action in _actions) {
        imageWidth = 0.f;
        titleLeftEdge = 0.f;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    FDPopoverViewCell *cell = [tableView dequeueReusableCellWithIdentifier:kPopoverCellIdentifier];
    if (!cell) {
        cell = [[FDPopoverViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kPopoverCellIdentifier];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [UIView animateWithDuration:0.25f animations:^{
        ...
        FDPopoverAction *action = _actions[indexPath.row];
